...
namespace PA_DronePack
{
    public class PA_DroneController : MonoBehaviour
    {
        ...
        [Range(0, 1f)][Tooltip("how fast the propellers slow down")]
        ...
        void FixedUpdate()
        {
            ...
            if (motorOn)  // if the drone's motor is on...
            {
                ...
                {                                                                                                                         //
                    if (groundDistance > 0.2f)                                                                                            // if we're NOT too close to the ground...
                    {                                                                                                                     //
                        if (driveInput > 0) { rigidBody.AddForceAtPosition(Vector3.down, frontTilt.position, ForceMode.Acceleration); }   // add downward/tilt force onto specified 'tilt points'
                        if (driveInput < 0) { rigidBody.AddForceAtPosition(Vector3.down, backTilt.position, ForceMode.Acceleration); }    // ...
                        if (strafeInput > 0) { rigidBody.AddForceAtPosition(Vector3.down, rightTilt.position, ForceMode.Acceleration); }  // ...
                        if (strafeInput < 0) { rigidBody.AddForceAtPosition(Vector3.down, leftTilt.position, ForceMode.Acceleration); }   // ...
                    }
